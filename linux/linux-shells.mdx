...
- STDIN (**0**): In der Konsole ist unsere Eingabe STDIN. Wir geben unseren Text in die STDIN unserer Shell ein, welche dann Programme ausführen kann, basierend auf unserem Input.
- STDOUT (**1**): Standardmäßig unser Output im Terminal. Wenn ein Programm Text in die Konsole schreibt, dann landet dieser in der Regel als regulärer Output in der Konsole.
- STDERR (**2**): Der Fehleroutput. Wird für Fehlermeldungen genutzt. Ist getrennt vom STDOUT, da man für Logging diese Fehler geg. in einer anderen Datei speichern möchte. Auch der Output dieses Streams landet standardmäßig in der Konsole.
